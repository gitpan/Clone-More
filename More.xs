...
#define SV_HOOK_OBJECT(s,c) do{\
	...
	if ( watch_hooks ) {\
		GV * clone_hook = gv_fetchmethod_autoload( 	SvSTASH( SvRV( source ) ), "CLONEMORE_clone", FALSE );\
		if ( clone_hook ) {\
			dSP;\
			int count;\
			ENTER;\
			...
			if ( SvTRUE( ERRSV ) ) {\
				...
				croak( "CLONEMORE_store did not return anticipated value; expected 1 return, got %d\n", count );\
			if ( ! SvROK( c ) )\
				croak( "CLONEMORE_store expected reference as return, got %d\n", SvTYPE( c ) );\
...
static SV * sv_clone( SV * source ) {
	...
	if ( SvREFCNT( source ) > 1 ) {
		...
		if ( CHECK_CIRCLE() && sv_is_circular( source ) )
			 return build_circular_return( source, (I32)SvIVX(perl_get_sv( "Clone::More::CIRCULAR_ACTION", TRUE ) ) );
#endif
#ifdef MINDFUL_REFS
...
MODULE = Clone::More		PACKAGE = Clone::More
...
void cloneXS( source )
	...
	PPCODE:
#ifdef MINDFUL_REFS
	break_refs      = ( SvTRUE( perl_get_sv( "Clone::More::BREAK_REFS",      TRUE ) ) );
#endif
#ifdef MINDFUL_CIR
	ignore_circular = ( SvTRUE( perl_get_sv( "Clone::More::IGNORE_CIRCULAR", TRUE ) ) );
#endif
#ifdef ALLOW_HOOKS
	watch_hooks     = ( SvTRUE( perl_get_sv( "Clone::More::ALLOW_HOOKS", TRUE ) ) );
#endif
...
void circularXS( source )
	...
	// Simply want to return a '1' or '0' so they can be evaluated boolean-ish
	is_circular = ( ( sv_deeply_circular( source ) ) ? newSVpv( "1", 0 ) : newSVpv( "0", 0 ) );
	hv_clear( sv_circle );
	sv_depth = 0;

	EXTEND( SP, 1 );
	PUSHs( sv_2mortal( is_circular ) );
